...
function createTablesIfNotExists(knex) {
	...
	return knex.schema.hasTable('users').then(function (exist) {
		if (exist) return knex.schema.dropTable('users');else return new Promise(function (resolve, reject) {
			return resolve();
		});
	}).then(function () {
		return knex.schema.hasTable('messages').then(function (exists) {
			if (exists) return knex.schema.dropTable('messages');else return new Promise(function (resolve, reject) {
				return resolve();
			});
		});
	}).then(function () {
		return knex.schema.hasTable('message_types').then(function (exists) {
			if (exists) return knex.schema.dropTable('message_types');else return new Promise(function (resolve, reject) {
				return resolve();
			});
		});
	}).then(function () {
		return knex.schema.hasTable('groupes').then(function (exists) {
			if (exists) return knex.schema.dropTable('groupes');else return new Promise(function (resolve, reject) {
				return resolve();
			});
		});
	}).then(function () {
		return knex.schema.hasTable('users_groupes').then(function (exists) {
			if (exists) return knex.schema.dropTable('users_groupes');else return new Promise(function (resolve, reject) {
				return resolve();
			});
		});
	}).then(function () {
		...
		var script_duration = stop - start;
